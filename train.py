...
def test_step(model: torch.nn.Module, 
  ...
  with torch.inference_mode():
      ...
      for batch, (X, y) in enumerate(dataloader):
          ...
          test_acc += accuracy(test_pred_logits, y).item()
...
def train(model: torch.nn.Module, 
  ...
  for epoch in tqdm(range(epochs)):
      ...
      torch.save(model, f"models/model_chk_{epoch}.pth")

  # Return the filled results at the end of the epochs
  torch.save(model, "models/f_model.pth")
  return results
...
if __name__=="__main__":
    ...
    optimizer = torch.optim.Adam(model.parameters(),
                             lr=LEARNING_RATE)
    train(model=model,train_dataloader=train_dataloader,test_dataloader=test_dataloader,optimizer=optimizer,loss_fn=loss_fn,epochs=NUM_EPOCHS)